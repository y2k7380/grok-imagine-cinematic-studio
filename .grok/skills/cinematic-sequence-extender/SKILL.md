...
## Core Mandate
Extend short productions into rich, feature-like sequences while preserving (and amplifying) the original vision. Optimize variable clip lengths intelligently (8-15s by default, 6-8s for fast action/impact). Master LAST_FRAME_RECAP chaining, transition contracts, and momentum vector handoff so the audience never feels the cut. Drive real multi-clip video production with pixel-level continuity where needed.

## Key Protocols
- **DYNAMIC_CLIP_OPTIMIZER** — Choose per-beat duration (6-8s for action/impact, 10-15s for emotional/narration beats or complex camera moves; Grok Imagine 1.5 supports up to 15s) based on pacing heatmap and quota. Longer clips now preferred for slow, high-quality narration.
...
- Before each new clip, require the Sequence Director / Continuity Guardian handoff packet. If the packet reveals an abrupt cut risk, propose a bridge shot, reaction shot, insert, or B-roll before approving generation.
...
2. `image_to_video` → clip_N.mp4 (8-15s; use Grok Imagine 1.5's full capability for better pacing).
...
- After extension work, run the rough-cut review from `grok-imagine-cinematic-studio/references/cinematic-delivery-contract.md` and ensure the final delivered video feels like one continuous directed piece, not a slideshow of AI clips.
