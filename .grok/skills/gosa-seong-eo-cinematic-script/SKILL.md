...
version: 0.2.0 (Updated 2026-06 with 토사구팽 drift & text-mixing lessons)
---
...
3. Output the complete script using the precise structure in `references/output-format.md`.
4. Craft every per-scene image and video prompt in detailed, cinematic English.
5. Include a strong 오프닝 훅, emotional 클라이맥스, satisfying 결말, and a practical 현대적 교훈.
6. Add a 10초 쇼츠용 요약 at the end.
...
- Main dramatic scenes (totaling the target runtime, using 6s or 10s clips for proper narration pacing).
...
- Use slow natural dramatic/educational pace with clear pauses (especially for 10s clips).
